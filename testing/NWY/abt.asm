...
.const
	aWidth			equ	310	; * Ширина окна - (Всётаки желательно, чтобы картинка имела такие размеры)
	aHeight			equ	190	; * Высота окна

	aStandartDelayTime	equ	2000	; * Стандартное время для задержки "блока текста" на экране

	aStartXPos			equ	10	; * Позиция по Х, откуда начинать выводить первый символ "блока строк"
	aStartYPos			equ	-50	; * Позиция по Y, откуда начинать выводить первый символ "блока строк"
...
.data
	...
	szaText	db	13,13,13,13						; <- Чередованием символов #13(опускаю строку вниз)
			;db	"[     The Power Of Reversing     ]",13	
				; <- Чередованием символов #13(опускаю строку вниз)
			db	"[ La fin justifie les moyens ]",8	; '8' <- Показывает что первая строка закончилась
...
;#######################################
Init_Proc proc hWnd:DWORD, Wxx:DWORD, Wyy:DWORD
	...
	add Wyy,1000
	invoke CreateBitmap, Wxx, Wyy, 1, 32, NULL
	mov ahBmp, eax
	...
	; Set Color of Text
	invoke SetTextColor, ahDC, RGB(65,67,89);0FFFFFFh
...
drawYolo proc hWnd:DWORD, Wxx:DWORD, Wyy:DWORD
		;push hWnd
		;pop aWnd
